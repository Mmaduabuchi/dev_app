...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    try{
        ...
        // Fetch current logo picture path to delete old file if exists
        $stmt = $conn->prepare("SELECT company_logo FROM `employer_profiles` WHERE id = ?");
        if (!$stmt) {
            throw new Exception('Database error: ' . $conn->error);
        }
        $stmt->bind_param("i", $userId);
        ...
        $currentPicturePath = $pictureRow['company_logo'] ?? null;
        $stmt->close();
        ...
        //get file extension
        $pictureExt  = strtolower(pathinfo($CompanyLogo['name'], PATHINFO_EXTENSION));
        // Generate unique file name
        $pictureName = uniqid('logo_', true) .  time() . '.' . $pictureExt;
        //create path
        $picturePath = $uploadDir . $pictureName;

        // Move the uploaded file to the designated directory
        if (!move_uploaded_file($CompanyLogo['tmp_name'], $picturePath)) {
            throw new Exception('Failed to move uploaded file.');
        }
        // Update the user's profile picture path in the database
        $relativePicturePath = 'library/documents/' . $pictureName;
        $stmt = $conn->prepare("UPDATE `employer_profiles` SET company_logo = ? WHERE user_id = ?");
        if (!$stmt) {
            throw new Exception('Database error: ' . $conn->error);
        }
        ...
        // Successful response
        sendResponse('success', 'Company logo updated successfully.');
